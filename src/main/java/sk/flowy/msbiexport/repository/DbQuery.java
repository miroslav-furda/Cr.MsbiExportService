    ...
    public static String getAllDataForKlient(String tableName, Integer klientID) {
        ...
        switch(tableName) {
            ...
            case "log_prihlasenie" : return "SELECT log_prihlasenie.* FROM log_prihlasenie " +
                    "INNER JOIN uzivatel ON log_prihlasenie.id_uzivatel = uzivatel.id AND uzivatel.id_klient = " + klientID;
            case "prijemka" : return "SELECT prijemka.* FROM prijemka " +
                    "INNER JOIN uzivatel ON uzivatel.id = prijemka.id_uzivatel AND uzivatel.id_klient = " + klientID;
